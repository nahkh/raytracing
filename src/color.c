...
Color color_make(double r, double g, double b) { return (Color){r, g, b}; }
...
Color color_blend(const Color a, const Color b, double alpha) {
  ...
  // This is not the correct way of blending between colors. It does not correct
  // for brightness.
  return color_add(color_multiply_scalar(a, 1.0 - alpha),
                   color_multiply_scalar(b, alpha));
}
...
Color255 color_to_color255(const Color color) {
  return (Color255){color_value_to_255(color.r), color_value_to_255(color.g),
                    color_value_to_255(color.b)};
}
