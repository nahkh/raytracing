...
Color render_color(Scene* scene, Ray ray, int reflections) {
  ...
  for (i = 0; i < scene->renderable_count; ++i) {
    collision = calculate_collision(scene->renderables[i], ray);
    if (collision.hit &&
        (!best_collision.hit || collision.distance < best_collision.distance)) {
  ...
  if (best_collision.hit) {
    ...
    for (i = 0; i < scene->light_source_count; ++i) {
      if (scene->light_sources[i].has_direction) {
        direction_to_light =
            get_direction_to(scene->light_sources[i], best_collision.pos);
        ray_to_light = ray_make(&(best_collision.pos), &direction_to_light);
        if (!calculate_hit(scene, ray_to_light)) {
          alpha =
              vector_dot_product(&(ray_to_light.dir), &(best_collision.normal));
          if (alpha > 0.0) {
            light_color = color_add(
                light_color,
                color_multiply_scalar(scene->light_sources[i].color, alpha));
          }
        }
      } else {
...
void* scene_thread_calculate_colors(void* params) {
  ...
  while (input->state != STOPPING) {
    if (input->state == RENDERING) {
      for (x = input->thread_num; x < input->camera->width;
           x += input->thread_count) {
        for (y = 0; y < input->camera->height; ++y) {
          input->pixels[x + y * input->camera->width] =
              color_to_color255(render_color(
                  input->scene, camera_get_ray(input->camera, x, y), 5));
        }
      }
      input->state = IDLE;
    }
...
RenderingThreadPool* start_rendering_threads(size_t thread_count, Scene* scene,
                                             Camera* camera, Color255* pixels) {
  ...
  thread_pool->threads = (RenderingThreadInput*)malloc(
      sizeof(RenderingThreadInput) * thread_count);
  ...
  for (i = 0; i < thread_count; ++i) {
    ...
    if (pthread_create(&(thread_pool->threads[i].thread_id), NULL,
                       scene_thread_calculate_colors,
                       (void*)&(thread_pool->threads[i]))) {
