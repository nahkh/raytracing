...
Matrix matrix_multiply_matrix(const Matrix* m, const Matrix* n) {
    ...
    return matrix_make( vector_dot_product(&r1, &c1), vector_dot_product(&r1, &c2), vector_dot_product(&r1, &c3), 
...
Vector matrix_multiply_vector(const Matrix* m, const Vector* v) {
    ...
    Vector r2 = matrix_row_2(m);
    Vector r3 = matrix_row_3(m);
    return vector_make(vector_dot_product(v, &r1), vector_dot_product(v, &r2), vector_dot_product(v, &r3));
}

Matrix matrix_multiply_scalar(const Matrix* m, double a) {
    return matrix_make(m->a1 * a, m->a2 * a, m->a3 * a, m->b1 * a, m->b2 * a, m->b3 * a, m->c1 * a, m->c2 * a, m->c3 * a);
}
...
inline Matrix matrix_make(double a1, double a2, double a3, double b1, double b2, double b3, double c1, double c2, double c3) {
